...
Kanbanise.prototype.applyTemplateTicket = function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        tmp += this.templateTicket.replace(/\$\{id\}/gi, data[i].id)
            ...
            .replace('${priority}', ' ' + data[i].priority + ' ')
 ...
 * @param {Array} data The data to stick into the template
 */
Kanbanise.prototype.applyTemplateRow = function(title, id, cards) {
    return jQuery(this.templateRow.replace('${title}', title).replace('${id}', id).replace('${cards}', cards));
};
...
Kanbanise.prototype.init = function() {
    ...
    function loadApiKey(issues) {
        ...
        jQuery.ajax(redmineRoot + 'my/account', {
    ...
    function getIssues() {
        ...
        rows.each(function(index, value) {
            ...
            if (jQuery(value).children('.priority')[0].innerHTML == 'Immediate' && jQuery(value).children('.status')[0].innerHTML != 'Closed') {
                category = 'immediate';
                priority = 'immediate';
            }
    ...
    function drawBoard(issues) {
        ...
        var row1Content = self.applyTemplateTicket(issues['immediate']);
        ...
        $(div).append(self.applyTemplateRow('Immediate', 'row1', row1Content));
        ...
        $(div).append($('<div class=\'credits\'>Kanbanise ' + VERSION + ' - brought to you by <a href=\'http://www.boxuk.com/\'>Box UK</a>' + ' | <a href=\'http://github.com/boxuk/kanbanise-redmine/issues\'>Feedback</a></div>'));
    ...
    function addStyling() {
        ...
        '.severity-major {}' + 
        '.severity-moderate {}' + 
        '.Immediate {width:20%; float:left;margin-left:10px;}'
...
(function() {
    ...
    // loading kanbanise
    if (window.jQuery === undefined || window.jQuery.fn.jquery < MIN_JQUERY_VERSION || window.jQueryUI === undefined) {
        loadJQuery();
    } else {
        k.init();
    }
